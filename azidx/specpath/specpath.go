...
type Info struct {
	...
	// subservices is used to store the optional sub-servce after the RP if any
	subservices []string
}
...
// - compute/resource-manager/Microsoft.Compute/Compute/Cloudservice/stable/2022-04-04/cloudService.json
func SpecPathInfo(p string) (*Info, error) {
	...
	for i := 3; i < len(segs)-3; i++ {
		subservices = append(subservices, segs[i])
	}
...
func (info Info) ToPath() string {
	...
	for _, subservice := range info.subservices {
		segs = append(segs, subservice)
	}
	if info.IsPreview {
